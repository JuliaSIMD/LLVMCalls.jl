...
@inline zero_mask(::Union{Val{W},StaticInt{W}}) where {W} = EVLMask{W}(zero(mask_type(Val{W}())), 0x00000000)
...
```julia
julia> mask(Val(8), 1, 6) # starting with `i = 1`, if vector is of length 6, 6 lanes are on
Mask{8,Bool}<1, 1, 1, 1, 1, 1, 0, 0>

julia> mask(Val(8), 81, 93) # if `i = 81` and the vector is of length 93, we want all lanes on.
Mask{8,Bool}<1, 1, 1, 1, 1, 1, 1, 1>

julia> mask(Val(8), 89, 93) # But after `i += 8`, we're at `i = 89`, and now want just 5 lanes on.
