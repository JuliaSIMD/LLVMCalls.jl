...
@inline Base.:(-)(v::Vec{<:Any,<:NativeTypes}) = Base.FastMath.sub_fast(zero(v), v)
@inline Base.FastMath.sub_fast(v::Vec{<:Any,<:NativeTypes}) = Base.FastMath.sub_fast(zero(v), v)

@inline Base.inv(v::AbstractSIMD{W,<:FloatingTypes}) where {W} = Base.FastMath.div_fast(one(v), v)
